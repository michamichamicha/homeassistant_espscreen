...
def discover_screens(registry, states, devices, areas):
    ...
    boards = {}
    for item in registry:
        if item.get("platform") != "esphome" or item.get("original_name") not in NAME_GUITION_TYPE:
            continue
        kind = str(states.get(item['entity_id'], {}).get('state', '')).casefold()
        boards[item.get("device_id")] = 'jc8012p4a1' if 'jc8012' in kind else 'guition'
